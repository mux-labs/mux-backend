...
describe('LimitsService webhook events', () => {
  ...
  describe('checkLimits', () => {
    ...
    it('does not throw when webhook dispatch fails', async () => {
      webhookEmitter.emitLimitExceeded.mockRejectedValue(
        new Error('dispatch fail'),
      );
      prisma.walletLimit.findUnique.mockResolvedValue({
        walletId: 'w1',
        dailyLimit: 10000,
  ...
  describe('optional webhook emitter', () => {
    it('works without webhook emitter (undefined)', async () => {
      const serviceWithoutWebhook = new LimitsService(prisma);
      prisma.walletLimit.findUnique.mockResolvedValue(null);
      prisma.walletLimit.upsert.mockResolvedValue({
        walletId: 'w1',
        dailyLimit: 1000,
      ...
      await expect(
        serviceWithoutWebhook.setLimits('w1', 1000, 100),
